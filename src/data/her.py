...
class HindisghtExperienceReplay(object):

    def __init__(self, make_env_fn, replay_strategy, k):
        self.make_env_fn = make_env_fn

        self.env = make_env_fn()
        self.env.reset()

        #Params are achieved_goal, desired_goal and info
        self.reward_fn = self.env.compute_reward

        self.replay_strategy = replay_strategy
        self.k = k

    def make_sample_her_transitions(self):
